...
class LinkedAnimate extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            data: getPointPos(1800, 900, this.num).map(item => ({
                ...item,
    ...
    render() {
        ...
        return (
            <div className={`${className}-wrapper`}>
                {Array.isArray(this.props.children) ? this.props.children.map(c => <div style={{position:'relative',zIndex:10}}>c</div>) : <div style={{position:'relative',zIndex:10}}>{this.props.children}</div>}
                <div
