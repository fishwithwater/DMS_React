...
class ChangeNumberAndWaitNumberForm extends Component {
            ...
            <Form onSubmit={this.handleSubmit.bind(this)} className="change-number-form" style={{ padding: '0 50px' }}>
                ...
                <Form.Item>
                    {getFieldDecorator('number', {
                        ...
                        <Input prefix={<Icon type="info-circle" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder="初始数量" />
                    )}
                </Form.Item>
                <Form.Item>
                    {getFieldDecorator('waitNumber', {
                        initialValue: data.waitNumber,
                        rules: [{ required: true, message: '请输入使用数量' }],
                    })(
                        <Input prefix={<Icon type="info-circle" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder="使用数量" />
                    )}
                </Form.Item>
                <Form.Item>
...
const WrappedNormalChangeNumberAndWaitNumberForm = Form.create({ name: 'normal_change_number' })(ChangeNumberAndWaitNumberForm);
export { WrappedNormalChangeNumberAndWaitNumberForm }  